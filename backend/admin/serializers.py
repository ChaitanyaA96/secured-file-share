...
class UserAdminChangeSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ['id', 'email', 'first_name', 'last_name', 'mfa_enabled', 'mfa_secret', 'email_verified', 'is_active', 'is_superuser', 'is_staff']
        extra_kwargs = {'is_superuser': {'read_only': True}, 'is_staff': {'read_only': False}}
