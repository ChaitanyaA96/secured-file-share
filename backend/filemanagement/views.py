...
class ShareFileView(APIView):
    ...
    def post(self, request):
        ...
        expires_at = (
            timezone.now() + datetime.timedelta(hours=int(expires_in))
            if expires_in
            else None
        )
        ...
        shared_link = generate_shared_link(
            request.build_absolute_uri("/"), share.shared_link, public=False
        )

        # Send email notification
        email_subject = (
            f"{request.user.first_name} {request.user.last_name} Shared a file with you"
        )
        email_message = f"Shared file details:\n\nFile Name: {file_obj.name}\n\n"
        email_message += f"You can {share_type} the file through website \n\n"
        email_message += (
            f"Expires At: {expires_at if expires_at else 'No expiration'}\n"
        )

        recipient_email = shared_with if shared_with else request.user.email
        send_email(to=[recipient_email], subject=email_subject, message=email_message)
...
class PublicShareFileView(APIView):
    ...
    def post(self, request):
        ...
        shared_link = generate_shared_link(
            request.build_absolute_uri("/"), share.shared_link, public=True
        )
...
class GetPublicShareDetails(APIView):
    ...
    def get(self, request):
        ...
        user = request.user
        shareDetails = get_object_or_404(
            FileShare, file=file_id, public=True, shared_by=user
        )
        if shareDetails.expires_at and shareDetails.expires_at < timezone.now():
            return Response(
                {"error": "This link has expired. Please generate a new link."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AccessSharedFileForAuthenticatedUsers(APIView):
    ...
    def get(self, request, shared_link):
        ...
        if share.is_expired():
            return Response(
                {"error": "This link has expired."}, status=status.HTTP_400_BAD_REQUEST
            )

        # Validate that the link is not public and is shared with the current user
        if share.public:
            return Response(
                {"error": "This link is public. Use the public API."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if share.share_type == "view":
            ...
            response["Content-Disposition"] = (
                f'inline; filename="{share.file.name}"'  # Ensure "inline" for viewing
            )
            return response
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="{share.file.name}"'  # Allow "attachment" for download
            )
            return response

        return Response(
            {"error": "Invalid share type."}, status=status.HTTP_400_BAD_REQUEST
        )
...
class AccessSharedFileForPublicUsers(APIView):
    ...
    def get(self, request, shared_link, passphrase):
        try:
            ...
            if not share.public:
                return Response(
                    {"error": "This link is restricted to authenticated users."},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            if share.passphrase and share.passphrase != passphrase:
                return Response(
                    {"error": "Invalid passphrase."}, status=status.HTTP_403_FORBIDDEN
                )
...
class SharedFilesListView(APIView):
    ...
    def get(self, request):
        shared_files = FileShare.objects.filter(
            shared_with=request.user.email, file__isnull=False
        )
        response_data = []
